...
class Q6(unittest.TestCase):
    ...
    def setUp(self):
        ...
        products = random.choices(list(self.catalog.keys()),k=random.randint(1,3))
        self.order = [(product,random.randint(1,10)) for product in products]
        self.expected = sum(self.catalog[product]*quantity for product,quantity in self.order)

    def test_q6(self):
        actual = student.q6(self.catalog.copy(),self.order.copy())
